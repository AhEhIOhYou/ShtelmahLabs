...
namespace _6
{
    class Program
    {
        ...
        public class Company
        {
            ...
            public void Search(string className, int sValue)
            {
                switch (className)
                {
                    case "_6.Program+PerMinuteRate":
                        IEnumerable<Tariff> perMinuteRatesQuery =
                            from Tariff t in _tariffsList
                            where t.SubCount == sValue && t.GetType().ToString() == "_6.Program+PerMinuteRate"
                            select t;
                        foreach (var tariff in perMinuteRatesQuery)
                        {
                            Console.WriteLine(tariff.ToString());
                        }
                        break;
                    case "_6.Program+PerSecondRate":
                        IEnumerable<Tariff> perSecondRatesQuery =
                            from Tariff t in _tariffsList
                            where t.SubCount == sValue && t.GetType().ToString() == "_6.Program+PerSecondRate"
                            select t;
                        foreach (var tariff in perSecondRatesQuery)
                        {
                            Console.WriteLine(tariff.ToString());
                        }
                        break;
                                ...
                                case "PerMinuteRate":
                                    ...
                                    switch (searchClassProp)
                                    {
                                        case "SubCount":
                                            ...
                                            c1.Search("_6.Program+PerMinuteRate", Int32.Parse(searchValue));
                                            break;
                                    }
                                    break;
                                case "PerSecondRate":
                                    ...
                                    switch (searchClassProp)
                                    {
                                        case "SubCount":
                                            Console.Write("Искомое количество абонентов: ");
                                            searchValue = Console.ReadLine();
                                            c1.Search("_6.Program+PerSecondRate", Int32.Parse(searchValue));
                                            break;
                                    }
                                    break;
                                case "exit":
